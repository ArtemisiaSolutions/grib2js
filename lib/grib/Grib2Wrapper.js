...
module.exports = function Grib2(pathToWGrib2) {
    ...
    function getLevelRequest(level) {
        ...
        } else if(level.match(/^[0-9]+$/)){
            return ["-match", level+" m"]
        } else {
            return []
        }
    }
    ...
    function getDataTypeRequest(dataType) {
        if(dataType == 'temperature' || dataType == 'TMP' ) {
            return ["-match", '"TMP"']
        } else if(dataType == 'snow' || dataType == 'WEASD') {
			return ["-match", "WEASD"]
	...
	this.getFile = function(date){
		...
		if(!fs.existsSync(file)) {
			...
			return "./gribs/20130215.grb2"
		}
		return 
	}

    this.getData = function(dataType, date, level, bounds, callback) {
		var csvFile = "./csv/"+shortener(date)+level+dataType+".csv"
	
		if(fs.existsSync(file))
		{
			console.log("Fichier CSV deja dispo")
			switch(dataType)
			{
				case "wind" :
		...
		{
			console.log("Fichier CSV non dispo")
...
/*
    this.getDataFromSpreadSheet = function(file, bounds, callback) {
        fs.readFile(file, "utf-8", function(err, data) {
            ...
            data.split("\n").forEach(function(line) {
                ...
                var lineData = {}
                lineData.lat = parseFloat(lineElements[0])
                lineData.lon = parseFloat(lineElements[1])
                lineData.value = parseFloat(lineElements[2])

                if(self.isIn(bounds, lineData) && lineData.value > 0) {
                    lineData.value -= 273.15
                    res.values.push(lineData)
                    if(!min || lineData.value<min) {
                        min = lineData.value
                    }
                    if(!max || lineData.value>max) {
                        max = lineData.value
                    }
                }
            })
...
*/
        ...
        fs.readFile(file, "utf-8", function(err, data) {
            ...
            data.split("\n").forEach(function(line) {
				...
				if(self.isIn(bounds, lineData.lat, lineData.lon)) {
					...
					lineData.value = parseFloat(lineElements[6])
    ...
    this.isIn = function(bounds, lat, lon) {
		...
		console.log(isInside ? "INSIDE" : "PAS INSIDE")
        return isInside
    }
