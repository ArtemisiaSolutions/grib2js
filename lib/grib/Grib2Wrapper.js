...
module.exports = function Grib2(pathToFile, pathToWGrib2) {
    ...
    this.getData = function(dataType, date, level, bounds, callback) {
        self.generateFileFor(dataType, date, level, shortener(date)+level+".csv", function(error, out) {
            if(error) return callback(error)
            return self.getDataFromFile(out, bounds, callback)
        })
    }

    this.generateFileFor = function(dataType, date, level, out, callback) {
        var queryParams = [self.file]
